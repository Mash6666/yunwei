...
function showSection(sectionId) {
    // 隐藏所有页面
    const sections = ['dashboard', 'metrics', 'alerts', 'history', 'ai-chat', 'database', 'config', 'knowledge-base'];
    sections.forEach(id => {
                ...
                messageContent += `
                    <div class="mt-3 p-2 bg-light border rounded">
                        <small class="text-muted">
                            <i class="fas fa-book"></i> 基于知识库回答 (检索到 ${result.retrieval_results} 个相关文档)
                        </small>
                    </div>
                `;
            }

            addChatMessage('ai', messageContent, result);
...
// 添加聊天消息到界面
function addChatMessage(type, message, metadata = null) {
...
function viewPlanDetails(planId) {
    ...
    if (commands.length > 0) {
        ...
        commands.forEach((cmd, index) => {
            commandsHtml += `
                <div class="command-item mb-3 p-3 bg-light rounded" data-command-index="${index}" data-plan-id="${planId}">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        ...
                        <div>
                            ...
                            <button class="btn btn-outline-primary btn-sm ms-2" onclick="toggleCommandEdit('${planId}', ${index})" title="编辑命令">
                    ...
                    <!-- 显示模式 -->
                    <div class="command-display" id="display-${planId}-${index}">
                    ...
                    <!-- 编辑模式 -->
                    <div class="command-edit" id="edit-${planId}-${index}" style="display: none;">
                        <div class="mb-2">
                            <label class="form-label">步骤描述:</label>
                            <input type="text" class="form-control form-control-sm" id="desc-${planId}-${index}" value="${cmd.description || ''}" placeholder="输入步骤描述">
                        </div>
                        <div class="mb-2">
                            <label class="form-label">执行命令:</label>
                            <textarea class="form-control form-control-sm font-monospace" id="cmd-${planId}-${index}" rows="3" placeholder="输入Shell命令">${cmd.command || ''}</textarea>
                        </div>
                        <div class="mb-2">
                            <label class="form-label">预期输出:</label>
                            <input type="text" class="form-control form-control-sm" id="output-${planId}-${index}" value="${cmd.expected_output || ''}" placeholder="输入预期输出">
                        </div>
                        <div class="mb-2">
                            <label class="form-label">超时时间(秒):</label>
                            <input type="number" class="form-control form-control-sm" id="timeout-${planId}-${index}" value="${cmd.timeout || 30}" min="1" max="300">
                        </div>
                        <div class="d-flex gap-2">
                            <button class="btn btn-success btn-sm" onclick="saveCommandEdit('${planId}', ${index})">
                                <i class="fas fa-save"></i> 保存
                            </button>
                            <button class="btn btn-secondary btn-sm" onclick="cancelCommandEdit('${planId}', ${index})">
...
// 命令编辑功能
function toggleCommandEdit(planId, commandIndex) {
    const displayDiv = document.getElementById(`display-${planId}-${commandIndex}`);
    const editDiv = document.getElementById(`edit-${planId}-${commandIndex}`);

    console.log(`切换编辑模式: planId=${planId}, index=${commandIndex}`);
    console.log('找到的元素:', { display: !!displayDiv, edit: !!editDiv });

    if (!displayDiv || !editDiv) {
        console.error('无法找到编辑表单元素');
        return;
    }
...
async function saveCommandEdit(planId, commandIndex) {
    console.log(`saveCommandEdit 被调用，planId=${planId}, index=${commandIndex}`);

    const descInput = document.getElementById(`desc-${planId}-${commandIndex}`);
    const cmdInput = document.getElementById(`cmd-${planId}-${commandIndex}`);
    const outputInput = document.getElementById(`output-${planId}-${commandIndex}`);
    const timeoutInput = document.getElementById(`timeout-${planId}-${commandIndex}`);
    ...
    if (!descInput || !cmdInput || !outputInput || !timeoutInput) {
        console.error('无法找到编辑表单的输入元素');
        showNotification('error', '错误', '无法找到编辑表单元素');
        return;
    }
    ...
    console.log('当前修复方案:', window.currentFixPlans);

    // 更新当前修复方案中的命令
    if (window.currentFixPlans && window.currentFixPlans.length > 0) {
        // 使用planId找到正确的修复方案
        let currentPlan = window.currentFixPlans.find(p => p.id === planId);

        // 如果没找到，尝试其他匹配方式
        if (!currentPlan) {
            // 尝试索引方式
            try {
                const planIndex = parseInt(planId.replace('plan_', '')) - 1;
                if (0 <= planIndex < window.currentFixPlans.length) {
                    currentPlan = window.currentFixPlans[planIndex];
                }
            } catch (e) {
                // 最后尝试通用查找
                currentPlan = window.currentFixPlans.find(p =>
                    p.commands && p.commands[commandIndex]
                );
            }
        }

        console.log('找到的修复方案:', currentPlan?.id);
        ...
        if (currentPlan && currentPlan.commands) {
            ...
            // 添加用户编辑标记
            currentPlan._user_edited = true;
            currentPlan._last_edit_time = new Date().toISOString();

            // 同步到后端状态管理器
            try {
                console.log('开始发送API请求...');
                console.log('发送的数据:', window.currentFixPlans);

                const response = await fetch('/api/save-fix-plans', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                    body: JSON.stringify({
                        fix_plans: window.currentFixPlans
                    })
                });
                ...
                if (result.success) {
                    console.log('保存成功，开始更新UI...');
                    // 更新显示 - 使用planId和commandIndex来精确定位
                    const descSpan = document.querySelector(`[data-plan-id="${planId}"][data-command-index="${commandIndex}"] .command-description`);
                    if (descSpan) {
                        descSpan.textContent = currentPlan.commands[commandIndex].description;
                    }

                    const codeElement = document.querySelector(`[data-plan-id="${planId}"][data-command-index="${commandIndex}"] .command-code code`);
                    if (codeElement) {
                        codeElement.textContent = currentPlan.commands[commandIndex].command;
                    }

                    const badgeElement = document.querySelector(`[data-plan-id="${planId}"][data-command-index="${commandIndex}"] .badge`);
                    if (badgeElement) {
                        badgeElement.textContent = `超时: ${currentPlan.commands[commandIndex].timeout}秒`;
                    }

                    showNotification('success', '保存成功', '命令已更新并同步到后端');
                    toggleCommandEdit(planId, commandIndex);
                } else {
                    console.error('API返回失败:', result.error);
                    showNotification('error', '保存失败', result.error || '同步到后端失败');
                }
            } catch (error) {
                console.error('保存修复方案失败:', error);
                showNotification('error', '保存失败', '网络错误或服务器异常');
            }
        }
    }
}

function cancelCommandEdit(planId, commandIndex) {
    const descInput = document.getElementById(`desc-${planId}-${commandIndex}`);
    const cmdInput = document.getElementById(`cmd-${planId}-${commandIndex}`);
    const outputInput = document.getElementById(`output-${planId}-${commandIndex}`);
    const timeoutInput = document.getElementById(`timeout-${planId}-${commandIndex}`);
...
// 显示知识库信息
function showKnowledgeBaseInfo() {
    refreshKnowledgeBaseStatus();
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">
                        <i class="fas fa-book"></i> 知识库状态
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body" id="knowledgeBaseModalContent">
                    <div class="text-center">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">关闭</button>
                    <button type="button" class="btn btn-primary" onclick="refreshKnowledgeBaseStatus()">刷新</button>
                </div>
            </div>
        </div>
    `;
...
async function refreshKnowledgeBaseStatus() {
    try {
        ...
        const statusHtml = `
            <div class="row">
                <div class="col-6">
                    <div class="card text-center">
                        <div class="card-body">
                            <h5 class="card-title text-primary">${result.document_count}</h5>
                ...
                <div class="col-6">
                    <div class="card text-center">
                        <div class="card-body">
                            <h5 class="card-title text-info">${result.collection_name}</h5>
                            <p class="card-text">集合名称</p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="mt-3">
                <small class="text-muted">
                    <i class="fas fa-folder"></i> 存储路径: ${result.persist_directory}<br>
                    <i class="fas fa-clock"></i> 最后更新: ${new Date(result.last_updated).toLocaleString('zh-CN')}
                </small>
            </div>
        `;
        ...
        // 更新模态框内容（如果存在）
        const modalContent = document.getElementById('knowledgeBaseModalContent');
        if (modalContent) {
        ...
        const errorHtml = `
            <div class="alert alert-danger">
                <i class="fas fa-exclamation-triangle"></i> 获取知识库状态失败
            </div>
        `;
...
async function initializeKnowledgeBase() {
    const folderPath = document.getElementById('documentFolder').value;

    if (!folderPath) {
        showNotification('warning', '参数错误', '请输入文档文件夹路径');
        return;
    }

    try {
        showNotification('info', '初始化中', '正在初始化知识库，请稍候...');
        ...
        const response = await fetch('/api/knowledge-base/initialize', {
            ...
            body: `document_folder=${encodeURIComponent(folderPath)}`
        });

        const result = await response.json();

        if (result.success) {
            showNotification('success', '初始化成功', result.message);
            refreshKnowledgeBaseStatus();
        } else {
            showNotification('error', '初始化失败', result.message);
        }

    } catch (error) {
        console.error('初始化知识库失败:', error);
        showNotification('error', '初始化失败', '网络连接错误');
    }
}

// 重置知识库
async function resetKnowledgeBase() {
    if (!confirm('确定要重置知识库吗？这将删除所有已索引的文档！')) {
        return;
    }

    try {
        const response = await fetch('/api/knowledge-base/reset', {
            method: 'POST'
        });
        ...
        if (result.success) {
            showNotification('success', '重置成功', result.message);
            refreshKnowledgeBaseStatus();
        } else {
            showNotification('error', '重置失败', result.message);
        }

    } catch (error) {
        console.error('重置知识库失败:', error);
        showNotification('error', '重置失败', '网络连接错误');
    }
}
...
async function testKnowledgeBaseSearch() {
    ...
    if (!query) {
        showNotification('warning', '参数错误', '请输入搜索查询');
        return;
    }

    try {
        const response = await fetch('/api/knowledge-base/search?query=' + encodeURIComponent(query) + '&k=5', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            }
        ...
        console.log('搜索结果:', result); // 调试日志

        if (result.success && result.results && result.results.length > 0) {
            resultsContent.innerHTML = `
                <div class="alert alert-success">
                    <i class="fas fa-check-circle"></i> 找到 ${result.total_results} 个相关文档
                </div>
                        ...
                        <div class="d-flex justify-content-between align-items-start">
                            <div class="flex-grow-1">
                                <h6 class="card-subtitle mb-2 text-muted">
                                    <i class="fas fa-file-alt"></i> 相关文档 ${index + 1}
                                    <span class="badge bg-info ms-2">相似度: ${(item.relevance_score * 100).toFixed(1)}%</span>
                                    <span class="badge bg-secondary ms-2">距离: ${item.distance ? item.distance.toFixed(4) : 'N/A'}</span>
                                </h6>
                                <p class="card-text">${item.content ? (item.content.length > 300 ? item.content.substring(0, 300) + '...' : item.content) : '无内容'}</p>
                                <small class="text-muted">
                                    <i class="fas fa-info-circle"></i> 来源: ${item.metadata && item.metadata.source ? item.metadata.source : '未知'}
                                    ${item.metadata && item.metadata.added_at ? ` | 添加时间: ${new Date(item.metadata.added_at).toLocaleString('zh-CN')}` : ''}
                                </small>
                            </div>
                        </div>
                    </div>
            ...
            resultsContent.innerHTML = `
                <div class="alert alert-warning">
                    <i class="fas fa-exclamation-triangle"></i> 未找到相关文档
                    <br><small class="text-muted">
                        查询: "${result.query || query}" |
                        总结果数: ${result.total_results || 0} |
                        ${result.error ? '错误: ' + result.error : '请尝试其他关键词'}
                    </small>
                </div>
            `;
        }
        ...
        resultsContent.innerHTML = `
            <div class="alert alert-danger">
                <i class="fas fa-exclamation-triangle"></i> 搜索请求失败
                <br><small class="text-muted">
                    错误信息: ${error.message}
                    <br>请检查网络连接或联系管理员
                </small>
        ...
        resultsDiv.style.display = 'block';
        showNotification('error', '搜索失败', '网络连接错误');
    }
}

// 测试RAG对话
async function testRAGChat() {
    const input = document.getElementById('ragTestInput');
    const message = input.value.trim();

    if (!message) return;

    // 添加用户消息
    const chatContainer = document.getElementById('ragTestChat');
    ...
    const userMessage = `
        <div class="d-flex justify-content-end mb-3">
            <div class="bg-primary text-white rounded p-2" style="max-width: 80%;">
                <small class="d-block text-end">${new Date().toLocaleTimeString('zh-CN')}</small>
                ${message}
            </div>
        </div>
    `;
    ...
    `;
    chatContainer.innerHTML += thinkingMessage;
    chatContainer.scrollTop = chatContainer.scrollHeight;
    ...
    try {
        ...
        // 移除思考状态
        const thinkingElement = chatContainer.querySelector('.spinner-border').closest('.d-flex');
        if (thinkingElement) {
        ...
        if (result.success) {
            const aiMessage = `
                <div class="d-flex justify-content-start mb-3">
                    <div class="bg-light rounded p-2" style="max-width: 80%;">
                        <small class="d-block text-muted">${new Date().toLocaleTimeString('zh-CN')}</small>
                        ${result.answer}
                        ${result.rag_used ? `
                            <div class="mt-2">
                                <small class="text-info">
                                    <i class="fas fa-book"></i> 基于知识库 (${result.retrieval_results} 个相关文档)
                                </small>
                            </div>
                        ` : ''}
                    </div>
            ...
            const errorMessage = `
                <div class="d-flex justify-content-start mb-3">
                    <div class="bg-danger text-white rounded p-2" style="max-width: 80%;">
                        <small class="d-block">${new Date().toLocaleTimeString('zh-CN')}</small>
                        ${result.error_message || '处理失败'}
                    </div>
                </div>
            `;
