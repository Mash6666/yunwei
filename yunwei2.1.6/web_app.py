...
# 导入RAG相关组件
from vector_database import get_vector_database, initialize_knowledge_base
...
async def chat_with_ai(request: ChatRequest):
    ...
    try:
        # 如果用户请求使用知识库检索，则使用RAG引擎
        if request.use_knowledge_base:
            rag_engine = get_rag_engine()
            result = rag_engine.process_message(
            ...
            if result["success"]:
                ...
                return {
                    "success": False,
                    "error": result.get("error", "RAG处理失败"),
                    "response": "抱歉，知识库检索失败，请稍后重试。",
                    "timestamp": result["timestamp"],
                    "rag_used": False
                }
        else:
            # 使用原有的React处理器
            return await react_chat_handler.handle_chat(request.message, request.use_knowledge_base)
...
async def approve_fix_plan(request: FixPlanRequest):
    ...
    try:
        ...
        # 调试信息：打印状态管理器中的所有方案
        logger.info(f"执行方案时的状态调试:")
        logger.info(f"状态管理器中的方案数量: {len(fix_plans)}")
        for i, plan in enumerate(fix_plans):
            logger.info(f"方案{i+1} ID: {plan.get('id', 'N/A')}")
            logger.info(f"  用户编辑标记: {plan.get('_user_edited', False)}")
            if 'commands' in plan:
                for j, cmd in enumerate(plan['commands']):
                    logger.info(f"  命令{j+1}: {cmd.get('command', 'N/A')}")
...
"""
        ...
        # 添加调试日志
        logger.info(f"LLM分析响应长度: {len(analysis_text)} 字符")
        logger.info(f"LLM分析响应前500字符: {analysis_text[:500] if len(analysis_text) > 500 else analysis_text}")

        # 解析分析结果
        parsed_result = analyzer._parse_analysis_result(analysis_text)
        logger.info(f"解析后的结果: {json.dumps(parsed_result, ensure_ascii=False, indent=2)[:1000]}")
        logger.info(f"修复方案数量: {len(parsed_result.get('fix_plans', []))}")
...
async def save_fix_plans(request: SaveFixPlansRequest):
    ...
    try:
        ...
        # 打印调试信息
        logger.info(f"收到前端修复方案: {len(fix_plans)} 个")
        for i, plan in enumerate(fix_plans):
            logger.info(f"方案{i+1} 用户编辑标记: {plan.get('_user_edited', False)}")
            if 'commands' in plan:
                for j, cmd in enumerate(plan['commands']):
                    logger.info(f"  命令{j+1}: {cmd.get('command', 'N/A')}")
        ...
        # 验证保存是否成功
        saved_state = ops_assistant.state_manager.get_state()
        saved_plans = saved_state.get('fix_plans', [])
        logger.info(f"状态管理器中保存的方案数量: {len(saved_plans)}")
        ...
        logger.info(f"成功保存了 {len(fix_plans)} 个修复方案到状态管理器")
...
async def upload_document_to_knowledge_base(request: DocumentUploadRequest):
    """上传文档到知识库"""
    try:
        db = get_vector_database()
        success = db.load_and_add_file(request.file_path)

        if success:
            return {
                "success": True,
                "message": f"文档 {request.file_path} 已成功添加到知识库",
                "timestamp": datetime.now().isoformat()
            }
        else:
            return {
                "success": False,
                "message": f"添加文档 {request.file_path} 失败",
                "timestamp": datetime.now().isoformat()
            }
...
async def initialize_knowledge_base_api(request: Request):
    """初始化知识库"""
    try:
        # 解析表单数据
        form_data = await request.form()
        document_folder = form_data.get("document_folder", "./documents")

        # 记录调试信息
        logger.info(f"知识库初始化请求，文档目录: {document_folder}")

        success = await initialize_knowledge_base(document_folder)
...
async def search_knowledge_base(query: str, k: int = 5):
    """搜索知识库"""
    try:
        logger.info(f"知识库搜索查询: {query} (k={k})")

        db = get_vector_database()
        results = db.similarity_search(query, k)

        logger.info(f"搜索结果: 找到 {len(results)} 个相关文档")
...
# 简单的测试端点
@app.post("/api/knowledge-base/search-test")
async def search_knowledge_base_test():
    """测试搜索知识库"""
    try:
        # 硬编码测试查询
        test_query = "衣服质量"

        logger.info(f"测试知识库搜索查询: {test_query}")

        db = get_vector_database()
        results = db.similarity_search(test_query, 5)

        logger.info(f"测试搜索结果: 找到 {len(results)} 个相关文档")
...
@app.post("/api/knowledge-base/chat", response_model=RAGChatResponse)
async def chat_with_knowledge_base(request: RAGChatRequest):
    """使用知识库进行对话"""
    try:
        rag_engine = get_rag_engine()
        result = rag_engine.process_message(
            ...
            force_rag=request.use_knowledge_base
        )
        ...
        return RAGChatResponse(
            ...
            retrieval_results=result.get("retrieval_results", 0),
        ...
        logger.error(f"知识库对话失败: {e}")
        return RAGChatResponse(
            success=False,
            answer=f"抱歉，处理请求时遇到错误: {str(e)}",
            sources=[],
            context_used=False,
            retrieval_results=0,
            rag_used=False,
...
async def startup_event():
    ...
    try:
        ...
        print(f"[RAG] 向量数据库初始化成功")
        print(f"[RAG] 知识库路径: {stats.get('persist_directory', 'N/A')}")
        print(f"[RAG] 文档数量: {stats.get('document_count', 0)}")

        # 初始化RAG引擎
        rag_engine = get_rag_engine()
        print("[RAG] RAG引擎初始化成功")

        # 尝试从默认文档文件夹初始化知识库
        docs_folder = "./documents"
        if os.path.exists(docs_folder):
            await initialize_knowledge_base(docs_folder)
            print(f"[RAG] 已从 {docs_folder} 加载文档到知识库")
        else:
            print(f"[RAG] 文档文件夹 {docs_folder} 不存在，请在需要时手动初始化知识库")

    except Exception as e:
        print(f"[RAG] RAG组件初始化失败: {e}")
        print("[RAG] 知识库功能将不可用，但其他功能正常")
