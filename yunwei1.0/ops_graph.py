    ...
    async def _analyze_system(self, state: OpsAssistantState) -> OpsAssistantState:
        ...
        try:
            ...
            self.state_manager.add_action("analyze_system", {
                "analysis_urgency": analysis_result.get("urgency"),
                "auto_fixable": analysis_result.get("auto_fixable"),
                "issues_count": len(analysis_result["detected_issues"]),
                "timestamp": datetime.now().isoformat()
            })
            ...
            logger.info(f"系统分析完成，检测到 {len(analysis_result['detected_issues'])} 个问题")
            return self.state_manager.get_state()
