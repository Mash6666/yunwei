...
function showSection(sectionId) {
    // 隐藏所有页面
    const sections = ['dashboard', 'metrics', 'alerts', 'history', 'ai-chat', 'config'];
    sections.forEach(id => {
...
// 启用命令编辑
function enableCommandEdit(planId, commandIndex) {
    const displayArea = document.getElementById(`command-display-${planId}-${commandIndex}`);
    const editArea = document.getElementById(`command-edit-${planId}-${commandIndex}`);
    const editBtn = document.getElementById(`edit-btn-${planId}-${commandIndex}`);

    if (displayArea && editArea && editBtn) {
        displayArea.style.display = 'none';
        editArea.style.display = 'block';
        editBtn.style.display = 'none';

        // 聚焦到编辑器
        const editor = document.getElementById(`command-editor-${planId}-${commandIndex}`);
        if (editor) {
            editor.focus();
        }
    }
...
async function saveCommandEdit(planId, commandIndex) {
    ...
    if (!editor) {
        showNotification('error', '错误', '找不到编辑器');
        return;
    }

    const newCommand = editor.value.trim();

    // 基本的命令验证
    if (!validateCommand(newCommand)) {
        showNotification('error', '命令验证失败', '命令包含不安全的内容，请检查后重试');
        return;
    }
    ...
    if (window.currentFixPlans) {
        const planIndex = window.currentFixPlans.findIndex(p =>
            p.id === planId || `plan_${window.currentFixPlans.indexOf(p) + 1}` === planId
        );

        if (planIndex !== -1 && window.currentFixPlans[planIndex].commands &&
            window.currentFixPlans[planIndex].commands[commandIndex]) {
            originalCommand = window.currentFixPlans[planIndex].commands[commandIndex].original_command ||
                            window.currentFixPlans[planIndex].commands[commandIndex].command;
        }
    }

    // 显示保存状态
    const saveBtn = editArea.querySelector('button[onclick*="saveCommandEdit"]');
    if (saveBtn) {
    ...
    try {
        // 调用后端API保存编辑
        const response = await fetch(window.location.origin + '/api/command/edit', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        if (result.success) {
            ...
            if (window.currentFixPlans) {
                ...
                if (planIndex !== -1 && window.currentFixPlans[planIndex].commands &&
                    ...
                    // 保存原始命令以备重置（如果后端没有保存）
                    if (!window.currentFixPlans[planIndex].commands[commandIndex].original_command) {
                        window.currentFixPlans[planIndex].commands[commandIndex].original_command = originalCommand;
                    }

                    window.currentFixPlans[planIndex].commands[commandIndex].command = newCommand;
                    window.currentFixPlans[planIndex].commands[commandIndex].user_modified = true;
                    window.currentFixPlans[planIndex].commands[commandIndex].modified_at = new Date().toISOString();
                }
            }
            ...
            if (displayArea) {
                displayArea.innerHTML = `
                    <div class="command-modified">
                        <pre class="mb-2 p-2 bg-dark text-white rounded"><code>${escapeHtml(newCommand)}</code></pre>
                    </div>
                `;
            }
            ...
            if (displayArea && editArea && editBtn) {
                ...
                editBtn.innerHTML = '<i class="fas fa-edit"></i> 重新编辑';
            }

            showNotification('success', '保存成功', result.message || '命令已成功修改并保存');
        } else {
            showNotification('error', '保存失败', result.message || '保存命令时发生错误');
        }
    } catch (error) {
        console.error('保存命令编辑失败:', error);
...
function cancelCommandEdit(planId, commandIndex) {
    ...
    // 恢复编辑器中的原始内容
    if (editor && window.currentFixPlans) {
        const planIndex = window.currentFixPlans.findIndex(p =>
            p.id === planId || `plan_${window.currentFixPlans.indexOf(p) + 1}` === planId
        );
        ...
        if (planIndex !== -1 && window.currentFixPlans[planIndex].commands &&
            ...
            const originalCommand = window.currentFixPlans[planIndex].commands[commandIndex].original_command ||
                                   window.currentFixPlans[planIndex].commands[commandIndex].command;
            editor.value = originalCommand;
        }
    }
...
// 重置命令编辑
function resetCommandEdit(planId, commandIndex) {
    const editor = document.getElementById(`command-editor-${planId}-${commandIndex}`);
    ...
    if (editor && window.currentFixPlans) {
        ...
        if (planIndex !== -1 && window.currentFixPlans[planIndex].commands &&
            ...
            showNotification('info', '重置成功', '命令已重置为原始AI生成内容');
        }
    }
}
...
function viewPlanDetails(planId) {
    ...
    if (commands.length > 0) {
        ...
        commands.forEach((cmd, index) => {
            commandsHtml += `
                <div class="command-item mb-3 p-3 bg-light rounded">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <strong>步骤 ${index + 1}: ${cmd.description || '执行命令'}</strong>
                        <span class="badge bg-info">超时: ${cmd.timeout || 30}秒</span>
                    </div>
                    <div class="command-code">
                        <div class="command-display-area" id="command-display-${planId}-${index}">
                            ${cmd.user_modified ? '<div class="edit-indicator"><i class="fas fa-edit"></i> 已修改</div>' : ''}
                            <pre class="mb-2 p-2 bg-dark text-white rounded"><code>${cmd.command || '无命令'}</code></pre>
                        </div>
                        <div class="command-edit-area mt-2" id="command-edit-${planId}-${index}" style="display: none;">
                            <div class="mb-2">
                                <label class="form-label small"><i class="fas fa-edit"></i> 编辑命令:</label>
                                <textarea class="form-control form-control-sm command-editor"
                                    id="command-editor-${planId}-${index}"
                                    rows="3"
                                    data-plan-id="${planId}"
                                    data-command-index="${index}"
                                    placeholder="在此编辑Shell命令...">${cmd.command || ''}</textarea>
                            </div>
                            <div class="d-flex gap-2 mb-2">
                                <button class="btn btn-success btn-sm" onclick="saveCommandEdit('${planId}', ${index})">
                                    <i class="fas fa-save"></i> 保存
                                </button>
                                <button class="btn btn-outline-secondary btn-sm" onclick="cancelCommandEdit('${planId}', ${index})">
                                    <i class="fas fa-times"></i> 取消
                                </button>
                                <button class="btn btn-outline-warning btn-sm" onclick="resetCommandEdit('${planId}', ${index})">
                                    <i class="fas fa-undo"></i> 重置
                                </button>
                            </div>
                            <div class="alert alert-warning small mb-2">
                                <i class="fas fa-exclamation-triangle"></i>
                                <strong>安全提醒:</strong> 请确保您了解所修改命令的作用，错误的命令可能导致系统问题
                            </div>
                        </div>
                        <div class="mt-2">
                            <button class="btn btn-outline-primary btn-sm" onclick="enableCommandEdit('${planId}', ${index})" id="edit-btn-${planId}-${index}">
