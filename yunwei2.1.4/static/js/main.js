...
async function testKnowledgeBaseSearch() {
    ...
    try {
        const response = await fetch('/api/knowledge-base/search', {
            ...
            body: `query=${encodeURIComponent(query)}&k=5`
        });
        ...
        if (result.success && result.results.length > 0) {
            resultsContent.innerHTML = result.results.map((item, index) => `
                <div class="card mb-2">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-start">
                            <div class="flex-grow-1">
                                ...
                                <small class="text-muted">
                                    <i class="fas fa-info-circle"></i> 来源: ${item.metadata.source || '未知'}
                                </small>
                            </div>
                        </div>
                    </div>
            ...
            resultsContent.innerHTML = `
                <div class="alert alert-info">
                    <i class="fas fa-info-circle"></i> 未找到相关文档
                </div>
            `;
        }
