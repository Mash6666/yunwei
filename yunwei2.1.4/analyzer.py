...
from logger_config import get_logger, error_logger
...
请基于提供的监控数据和系统信息，进行专业的分析和建议。"""
    ...
    def _build_analysis_prompt(self, context: Dict[str, Any]) -> str:
        """构建分析提示词"""
        # 提取关键指标，精简输入内容
        critical_metrics = self._extract_critical_metrics(context)

        prompt = f"""分析Linux系统状态，时间: {context['timestamp']}

## 关键数据概览
- 告警: {context['alerts_summary']['total_alerts']}个 (严重:{context['alerts_summary']['critical_alerts']})
- 关键指标: {critical_metrics}

## 活跃告警
{self._format_alerts(context['active_alerts'])[:500]}  # 限制告警信息长度

## 阈值参考
- CPU:{Config.THRESHOLDS['cpu_usage']}%, 内存:{Config.THRESHOLDS['memory_usage']}%, 磁盘:{Config.THRESHOLDS['disk_usage']}%

请提供简洁的分析和修复方案：
1. 系统状态总体评估
2. 检测到的具体问题及根因分析
3. 可执行的修复方案（包含具体的Shell命令）
4. 操作风险评估和优先级
5. 是否建议自动修复
6. 修复后需要进一步分析的内容
...
{{
    ...
    "recommendations": ["预防性建议1", "预防性建议2"],
    "urgency": "low|medium|high|critical",
    "auto_fixable": true|false,
    "next_analysis": "修复后需要进一步分析的内容",
    "impact_assessment": {{
        "affected_services": ["受影响的服务"],
        "potential_downtime": "预估停机时间",
        "data_risk": "数据风险等级"
    }}
}}

重要要求：
- commands字段必须提供具体可执行的Shell命令
- 每个修复计划都应包含完整的执行、验证和回滚方案
- 风险评估必须考虑系统稳定性和数据安全
- 预估时间应该包括执行和验证时间
- 回滚方案必须完整且可执行
"""
    ...
    def _extract_critical_metrics(self, context: Dict[str, Any]) -> str:
        """提取关键指标，精简输入内容"""
        critical_info = []

        # 提取关键CPU指标
        cpu_metrics = context['detailed_metrics']['cpu']
        if cpu_metrics:
            cpu_usage = [m for m in cpu_metrics if 'usage' in m['name'].lower() or 'percent' in m['name'].lower()]
            if cpu_usage:
                critical_info.append(f"CPU:{cpu_usage[0]['value']:.1f}%")

        # 提取关键内存指标
        memory_metrics = context['detailed_metrics']['memory']
        if memory_metrics:
            mem_usage = [m for m in memory_metrics if 'usage' in m['name'].lower() or 'percent' in m['name'].lower()]
            if mem_usage:
                critical_info.append(f"内存:{mem_usage[0]['value']:.1f}%")

        # 提取关键磁盘指标
        disk_metrics = context['detailed_metrics']['disk']
        if disk_metrics:
            disk_usage = [m for m in disk_metrics if 'usage' in m['name'].lower() or 'percent' in m['name'].lower()]
            if disk_usage:
                critical_info.append(f"磁盘:{disk_usage[0]['value']:.1f}%")

        # 提取关键系统负载
        system_metrics = context['detailed_metrics']['system']
        if system_metrics:
            load_metrics = [m for m in system_metrics if 'load' in m['name'].lower()]
            if load_metrics:
                critical_info.append(f"负载:{load_metrics[0]['value']:.2f}")

        return ", ".join(critical_info) if critical_info else "无关键指标数据"
    ...
    def create_fix_plan_from_analysis(self, analysis_result: Dict[str, Any]) -> List[str]:
        ...
        if "fix_plans" in analysis_result and analysis_result["fix_plans"]:
            ...
            if auto_fixable:
                ...
                # 如果不自动修复，添加到计划中等待人工确认
                for plan in sorted_plans:
                    plan_id = plan.get("id", f"plan_{len(execution_plan)}")
                    execution_plan.append(f"修复计划: {plan.get('description', plan_id)}")

        # 如果没有修复计划但有简单操作，使用原来的逻辑
